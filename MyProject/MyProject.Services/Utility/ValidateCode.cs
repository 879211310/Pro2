...
namespace MyProject.Services.Utility
{
    /// <summary>
    /// 生产验证码类 <br/>
    /// 该类使用说明：该类用于生成验证码，可以自定义验证码参数如：背景颜色，字体，宽度，高度等，<br/>
    /// 默认：字体为红色，字体为Verdana，14px字体大小，被背景为白色，相关参考源码。<br/>
    /// 使用事例子： <br/>
    /// ValidateCode vc = new ValidateCode(); //创建默认参数验证码<br/>
    /// //自定义验证码参数，模糊度,字体,背景颜色<br/>
    /// ValidateCode vc = new ValidateCode(5, new Font("Verdana", 14), Color.SandyBrown, Color.Yellow); <br/>
    /// // 调用CreateImage方法生成图片指定参数宽，高，OUT验证代码（用于自行将验证代码保存到Session）<br/>
    /// using (Image img = vc.CreateImage(80, 30, out code)) <br/>
    /// {<br/>
    ///   img.Save(context.Response.OutputStream, ImageFormat.Jpeg);<br/>
    /// }<br/>      
    /// </summary>
